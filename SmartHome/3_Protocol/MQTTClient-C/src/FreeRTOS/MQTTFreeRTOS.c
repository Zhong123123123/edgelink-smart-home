...
const static WiFiInfo wifi = {WiFi_ID, SH_WIFI_SSID, SH_WIFI_PASSWORD};
...
void NetworkInit(Network* n)
{
	...
	if(n->netdev != NULL)
	{
		if(n->netdev->Init(n->netdev) == 0)
		{
				...
				{
					LOG_INFO("NET", "connect wifi success");
				...
				{
					LOG_ERROR("NET", "connect wifi failed");
				}
			}
			else
			{
				LOG_ERROR("NET", "net device init failed");
			}
		}
		else
		{
			LOG_ERROR("NET", "net device not found");
		}
